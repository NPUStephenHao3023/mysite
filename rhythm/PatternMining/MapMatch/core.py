...
def get_transimission_probability(pre_closest_point, closest_point):
    ...
    max_distance = (closest_point.log_time -
                    pre_closest_point.log_time).total_seconds() * 33
    max_distance = max_distance if max_distance < MyTool.MAX_DIS else MyTool.MAX_DIS
    dijkstra_distance = MyTool.get_dijkstra_distance(
        pre_closest_point, closest_point, max_distance)
    ...
    if dijkstra_distance == MyTool.MAX_DIS:
...
def get_observation_probability(closest_point):
    ...
    '''
    dis = sp.distance.euclidean([closest_point.log_x, closest_point.log_y], [
                                closest_point.p_x, closest_point.p_y])
...
#            continue
        ...
        for closest_point_idx, closest_point in enumerate(closest_points):
            ...
            g.add_node(
                point_id, observation_probability=get_observation_probability(closest_point))
            if len(pre_layer) == 0:
                continue
            else:
                for pre_point_id in pre_layer:
                    ...
                    transimission_probability = get_transimission_probability(
                        pre_closest_point, closest_point)
                    g.add_edge(pre_point_id, point_id,
                               transimission_probability=transimission_probability)
...
def find_match_sequence(g, log_list, log_closest_points):
    ...
    for now_log_id in log_list[1:]:
        for now_closest_point_idx, now_closest_point in enumerate(log_closest_points[now_log_id]):
            ...
            for pre_closest_point_idx, pre_closest_point in enumerate(log_closest_points[pre_log_id]):
                pre_point_id = str(pre_log_id) + '_' + \
                    str(pre_closest_point_idx)
                temp = g[pre_point_id][now_point_id]['transimission_probability'] * \
                    g.node[now_point_id]['observation_probability'] + \
                    f[pre_point_id]
    ...
    assert(max_point_id.split('_')[0] == str(
        log_list[-1]))  # 断言， 概率最大的候选点， 一定在最后一组内
...
def match_until_connect(log_list, log_closest_points):
    ...
    while True:
        ...
        # begin_tick = time.time()
        is_connect, match_point_list, break_idx = find_match_sequence(
            g, log_list, log_closest_points)
