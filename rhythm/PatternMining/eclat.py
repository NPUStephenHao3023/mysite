...
def compute_vertical_bitvector_data(data, use_CUDA):
    ...
    # print('Data transformed into vertical bitvector representation with shape: ',
    #       np.shape(vb_data))
...
class eclat_runner:
    ...
    def run(self, prefix, supportK):
        ...
        #       len(self.support_list), end='\r')
        while supportK:
            itemset, bitvector = supportK.pop(0)
            support = np.sum(bitvector)

            if support >= self.min_support:
                self.support_list[frozenset(
                    sorted(prefix + [itemset]))] = int(support)
                ...
                self.run(prefix+[itemset], sorted(suffix,
    ...
    def cuda_run(self, prefix, supportK):
        ...
        while supportK:
            ...
            if support >= self.min_support:
                ...
                self.cuda_run(
                    prefix+[itemset], sorted(suffix, key=lambda x: int(x[0]), reverse=True))
...
def output_handling(support_list):
    ...
    if len(L) != 0:
        L[-1] = sorted(L[-1])
    L = tuple(L)
    support_list = dict((tuple(sorted(list(k), key=lambda x: int(x))), v)
...
def eclat(data, min_support, iterative=False, use_CUDA=False, block=None, thread=None):
    ...
    if iterative and not use_CUDA:
        ...
        vb_data, idx2item = compute_vertical_bitvector_data(
            data, use_CUDA=False)
        L1, support_list = compute_L1(
            vb_data, idx2item, num_trans, min_support)
        L = [L1]
        k = 1

        while True:
            print('Running Eclat: the %i-th iteration with %i itemsets in L%i...' %
                  (k, len(L[-1]), k))
            k += 1
            LK, supportK = compute_LK(
                L[-1], support_list, k, num_trans, min_support)

            if len(LK) == 0:
                L = [sorted([tuple(sorted(itemset)) for itemset in LK])
                     for LK in L]
                support_list = dict((tuple(sorted(k)), np.sum(v))
                                    for k, v in support_list.items())
    ...
    #---recursive method---#
        ...
        if use_CUDA and not CUDA_FLAG:
            use_CUDA = False
        vb_data, idx2item = compute_vertical_bitvector_data(
            data, use_CUDA=use_CUDA)
