...
def plot_opaque_cube(ax,x=10, y=20, z=30, dx=40, dy=50, dz=60,color='red'):
    ...
    ax.plot_surface(xx, yy, np.array([[z],[0]]),**kwargs)
    ax.plot_surface(xx, yy, np.array([[z+dz],[0]]),**kwargs)
    yy, zz = np.meshgrid(yy, zz)
    ax.plot_surface(np.array([[x],[0]]), yy, zz,**kwargs)
    ax.plot_surface(np.array([[x+dx],[0]]), yy, zz,**kwargs)
    xx, zz = np.meshgrid(xx, zz)
    # ax.plot_surface(xx, np.array([[y],[0]]), zz,**kwargs)
    ax.plot_surface(xx, y, zz,**kwargs)
    # ax.plot_surface(xx, np.array([[y+dy],[0]]), zz,**kwargs)
